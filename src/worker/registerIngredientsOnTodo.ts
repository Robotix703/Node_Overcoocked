...
export async function updateTodoItem(existingIngredient : ITodoItem[], newIngredient : IIngredientWithQuantity, recipeName: string) : Promise<void> {
  ...
  const underline : string = existingIngredient[0].underline;
  ...
  await Todoist.updateItem(existingIngredient[0].todoID, ingredientText, newUnderline);
  await baseTodoItem.updateTodoItem(existingIngredient[0]._id, existingIngredient[0].todoID, ingredientText, newIngredient.ingredient.name, existingIngredient[0].consumable, newUnderline);
}
